...
public final class HttpDetailsFxController {
    ...
    private List<Field<?>> initContextProperties(final XHttpComponentDTO httpComponent) {
        ...
        final Field<?> contextServiceIdField = Field.ofIntegerType(Math.toIntExact(httpComponent.contextServiceId))
                .editable(false).label("Service ID");
    ...
    private List<Field<?>> initErrorPageProperties(final XHttpComponentDTO errrorPage) {
        ...
        final Field<?> asyncSupportedField = Field.ofBooleanType(errrorPage.asyncSupported).editable(false)
                .label("Async Supported");
        final Field<?> serviceIdField      = Field.ofIntegerType(Math.toIntExact(errrorPage.serviceId)).editable(false)
                .label("Service ID");
        ...
        return Arrays.asList(nameField, asyncSupportedField, serviceIdField, infoField, exceptionsField,
                errorCodesField);
    }

    private List<Field<?>> initListenerProperties(final XHttpComponentDTO listener) {
        final Field<?> serviceIdField = Field.ofIntegerType(Math.toIntExact(listener.serviceId)).editable(false)
                .label("Service ID");
    ...
    private List<Field<?>> initResourceProperties(final XHttpComponentDTO resource) {
        ...
        final Field<?> serviceIdField = Field.ofIntegerType(Math.toIntExact(resource.serviceId)).editable(false)
                .label("Service ID");
    ...
    private List<Field<?>> initFilterProperties(final XHttpComponentDTO filter) {
        ...
        final Field<?> asyncSupportedField = Field.ofBooleanType(filter.asyncSupported).editable(false)
                .label("Async Supported");
        final Field<?> serviceIdField      = Field.ofIntegerType(Math.toIntExact(filter.serviceId)).editable(false)
                .label("Service ID");
        ...
        return Arrays.asList(nameField, asyncSupportedField, serviceIdField, patternsField, servletNamesField,
                regexsField, dispatcherField);
    }
    ...
    private List<Field<?>> initServletProperties(final XHttpComponentDTO servlet) {
        ...
        final Field<?> asyncSupportedField = Field.ofBooleanType(servlet.asyncSupported).editable(false)
                .label("Async Supported");
        final Field<?> serviceIdField      = Field.ofIntegerType(Math.toIntExact(servlet.serviceId)).editable(false)
                .label("Service ID");
