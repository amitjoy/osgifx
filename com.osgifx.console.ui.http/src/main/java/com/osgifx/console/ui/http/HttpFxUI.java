...
public final class HttpFxUI {
    ...
    @Inject
    @Optional
    private void updateControlsOnEvent(@UIEventTopic(CONFIGURATION_ACTION_EVENT_TOPICS) final String data,
    ...
    private void initStatusBar(final BorderPane parent) {
        ...
        if (isConnected) {
            ...
            final var isHttpAvailable = dataProvider.runtimeCapabilities().stream().filter(c -> "HTTP".equals(c.id))
                    .anyMatch(c -> c.isAvailable);
            if (isHttpAvailable && !isSnapshotAgent) {
