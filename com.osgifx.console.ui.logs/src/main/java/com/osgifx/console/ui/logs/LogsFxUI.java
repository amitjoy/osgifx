...
public final class LogsFxUI {
    ...
    @Inject
    @Optional
    private void updateOnLogReceiveStarted(@UIEventTopic(LOG_RECEIVE_STARTED_EVENT_TOPIC) final String data,
    ...
    @Inject
    @Optional
    private void updateControlsOnEvent(@UIEventTopic(BUNDLE_ACTION_EVENT_TOPICS) final String data,
                                       final BorderPane parent,
                                       @LocalInstance final FXMLLoader loader) {
        createControls(parent, loader);
    }
    ...
    private void initStatusBar(final BorderPane parent) {
        ...
        if (isConnected) {
            ...
            final var isR7LoggerAvailable = dataProvider.runtimeCapabilities().stream()
                    .filter(c -> "R7_LOGGER".equals(c.id)).anyMatch(c -> c.isAvailable);
            if (isR7LoggerAvailable && !isSnapshotAgent) {
