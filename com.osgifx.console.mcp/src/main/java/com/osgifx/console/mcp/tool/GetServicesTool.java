...
@McpToolDef(name = "osgi_get_services", description = "Returns the detailed information of all services")
public class GetServicesTool implements McpTool {
    ...
    public Object execute(final Map<String, Object> args) throws Exception {
        logger.atInfo().log("Executing GetServicesTool");
        final var agent = supervisor.getAgent();
        if (agent == null) {
            logger.atWarning().log("Agent is not connected");
            return Collections.emptyList();
        }
        final var services = agent.getAllServices();
        logger.atInfo().log("Retrieved services: %s", services.size());
        return services;
    }
}
