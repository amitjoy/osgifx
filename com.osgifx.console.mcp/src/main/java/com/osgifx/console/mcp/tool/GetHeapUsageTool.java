...
@McpToolDef(name = "osgi_get_heap_usage", description = "Returns the heap usage information")
public class GetHeapUsageTool implements McpTool {
    ...
    public Object execute(final Map<String, Object> args) throws Exception {
        logger.atInfo().log("Executing GetHeapUsageTool");
        final var agent = supervisor.getAgent();
        if (agent == null) {
            logger.atWarning().log("Agent is not connected");
            return null;
        }
        final var heap = agent.getHeapUsage();
        logger.atInfo().log("Retrieved heap usage");
        return heap;
    }
}
