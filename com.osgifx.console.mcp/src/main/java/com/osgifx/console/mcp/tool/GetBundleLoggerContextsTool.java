...
public class GetBundleLoggerContextsTool implements McpTool {
    ...
    public Object execute(final Map<String, Object> args) throws Exception {
        logger.atInfo().log("Executing GetBundleLoggerContextsTool");
        final var agent = supervisor.getAgent();
        if (agent == null) {
            logger.atWarning().log("Agent is not connected");
            return Collections.emptyList();
        }
        final var contexts = agent.getBundleLoggerContexts();
        logger.atInfo().log("Retrieved bundle logger contexts: %s", contexts.size());
        return contexts;
    }
}
