...
@McpToolDef(name = "osgi_get_configurations", description = "Returns the detailed information of all the configurations")
public class GetConfigurationsTool implements McpTool {
    ...
    public Object execute(final Map<String, Object> args) throws Exception {
        logger.atInfo().log("Executing GetConfigurationsTool");
        final var agent = supervisor.getAgent();
        if (agent == null) {
            logger.atWarning().log("Agent is not connected");
            return Collections.emptyList();
        }
        final var configurations = agent.getAllConfigurations();
        logger.atInfo().log("Retrieved configurations: %s", configurations.size());
