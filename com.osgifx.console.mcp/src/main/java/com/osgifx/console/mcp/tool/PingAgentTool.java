...
@McpToolDef(name = "ping_agent", description = "Pings the remote agent to verify connectivity and latency. Returns 'pong' if successful.")
public class PingAgentTool implements McpTool {
    ...
    public Object execute(final Map<String, Object> args) throws Exception {
        logger.atInfo().log("Executing PingAgentTool");
        final var agent = supervisor.getAgent();
        if (agent == null) {
            logger.atWarning().log("Agent is not connected");
            return "Agent not connected";
        }
        final var alive = agent.ping();
        return alive ? "pong" : "agent_down";
    }
