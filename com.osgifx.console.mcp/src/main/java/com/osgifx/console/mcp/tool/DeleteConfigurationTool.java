...
public class DeleteConfigurationTool implements McpTool {
    ...
    public Map<String, Object> inputSchema() {
        return McpToolSchema.builder().arg("pid", "string", "The configuration PID to delete").build();
    }

    @Override
    public Object execute(final Map<String, Object> args) throws Exception {
        logger.atInfo().log("Executing DeleteConfigurationTool");
        ...
        final var result = agent.deleteConfiguration(pid);

        if (result.result == XResultDTO.ERROR) {
            return "Error deleting configuration '" + pid + "': " + result.response;
        }
        return "Configuration '" + pid + "' deleted successfully";
    }
}
