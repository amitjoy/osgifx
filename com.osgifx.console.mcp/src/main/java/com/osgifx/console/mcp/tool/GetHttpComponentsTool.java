...
public class GetHttpComponentsTool implements McpTool {
    ...
    public Object execute(final Map<String, Object> args) throws Exception {
        logger.atInfo().log("Executing GetHttpComponentsTool");
        final var agent = supervisor.getAgent();
        if (agent == null) {
            logger.atWarning().log("Agent is not connected");
            return Collections.emptyList();
        }
        final var components = agent.getHttpComponents();
        logger.atInfo().log("Retrieved HTTP components: %s", components.size());
        return components;
    }
}
