...
@McpToolDef(name = "osgi_get_gogo_commands", description = "Returns the set of registered Gogo commands")
public class GetGogoCommandsTool implements McpTool {
    ...
    public Object execute(final Map<String, Object> args) throws Exception {
        logger.atInfo().log("Executing GetGogoCommandsTool");
        final var agent = supervisor.getAgent();
        if (agent == null) {
            logger.atWarning().log("Agent is not connected");
            return Collections.emptyList();
        }
        final var commands = agent.getGogoCommands();
        logger.atInfo().log("Retrieved Gogo commands: %s", commands.size());
        return commands;
    }
}
