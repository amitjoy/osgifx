...
@McpToolDef(name = "osgi_get_classloader_leaks", description = "Returns the list of suspicious bundles causing probable classloader leaks")
public class GetClassloaderLeaksTool implements McpTool {
    ...
    public Object execute(final Map<String, Object> args) throws Exception {
        logger.atInfo().log("Executing GetClassloaderLeaksTool");
        final var agent = supervisor.getAgent();
        if (agent == null) {
            logger.atWarning().log("Agent is not connected");
            return Collections.emptyList();
        }
        final var leaks = agent.getClassloaderLeaks();
        logger.atInfo().log("Retrieved classloader leaks: %s", leaks.size());
        return leaks;
    }
}
