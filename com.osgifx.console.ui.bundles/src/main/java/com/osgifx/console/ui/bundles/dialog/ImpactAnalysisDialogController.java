...
import com.osgifx.console.ui.bundles.model.ImpactAnalyzer;
import com.osgifx.console.ui.bundles.model.XImpactDTO;
...
public final class ImpactAnalysisDialogController {
                ...
                if (empty || item == null) {
                    ...
                    switch (item) {
                        ...
                        case "DEACTIVATION":
                            ...
                            break;
                        case "STALE_WIRING":
                            icon = new Glyph("FontAwesome", FontAwesome.Glyph.WARNING).color(Color.ORANGE);
                            break;
                        case "SERVICE_UNBIND":
    ...
    public void analyze(final String action,
                        final Collection<XBundleDTO> selection,
                        final Collection<XBundleDTO> allBundles,
                        final Collection<XComponentDTO> allComponents) {
        final var target = selection.size() == 1 ? selection.iterator().next().symbolicName
                : selection.size() + " bundles";
        ...
        if ("STOP".equalsIgnoreCase(action)) {
            ...
            headerIcon.setIcon(FontAwesome.Glyph.TRASH);
            headerIcon.setStyle("-fx-text-fill: #dc2626;");
        ...
        executor.supplyAsync(() -> {
            if ("STOP".equalsIgnoreCase(action) || "UNINSTALL".equalsIgnoreCase(action)) {
                return ImpactAnalyzer.calculateStopImpact(action, selection, allBundles, allComponents);
            }
            if ("START".equalsIgnoreCase(action)) {
                return ImpactAnalyzer.calculateStartImpact(selection, allBundles, allComponents);
            }
