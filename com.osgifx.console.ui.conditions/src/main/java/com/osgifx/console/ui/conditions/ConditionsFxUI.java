...
public final class ConditionsFxUI {
    ...
    public void postConstruct(final BorderPane parent, @LocalInstance final FXMLLoader loader) {
        ...
        logger.atDebug().log("Conditions part has been initialized");
    }
    ...
    @Inject
    @Optional
    private void onFilterUpdateEvent(@UIEventTopic(UPDATE_CONDITION_FILTER_EVENT_TOPIC) final SearchFilterDTO filter,
                                     final BorderPane parent) {
        logger.atInfo().log("Condition filter update event received");
        searchFilter = filter;
        ...
        if (isConnected) {
            ...
            if (searchFilter != null && searchFilter.predicate != null) {
                ...
                final var searchFilterResetNode = Fx
                        .initStatusBarButton(() -> FxDialog.showConfirmationDialog("Reset Search Filter?", description,
                                getClass().getClassLoader(), btn -> {
                                                                    if (btn == ButtonType.OK) {
                                                                        eventBroker.post(
                                                                                UPDATE_CONDITION_FILTER_EVENT_TOPIC,
                                                                                new SearchFilterDTO());
                                                                    }
                                                                }),
