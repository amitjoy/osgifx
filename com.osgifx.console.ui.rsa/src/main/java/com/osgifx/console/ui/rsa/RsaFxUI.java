...
public final class RsaFxUI {
    ...
    public void postConstruct(final BorderPane parent, @LocalInstance final FXMLLoader loader) {
        ...
        logger.atDebug().log("RSA part has been initialized");
    }
    ...
    private void initStatusBar(final BorderPane parent) {
        ...
        if (isConnected) {
            ...
            final var isRsaAvailable = dataProvider.runtimeCapabilities().stream().filter(c -> "RSA".equals(c.id))
                    .anyMatch(c -> c.isAvailable);
            if (isRsaAvailable && !isSnapshotAgent) {
