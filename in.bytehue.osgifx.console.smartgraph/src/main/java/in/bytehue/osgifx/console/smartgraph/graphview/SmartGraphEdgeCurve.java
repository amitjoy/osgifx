 ...
 * the constructor {@link #SmartGraphEdgeCurve(in.bytehue.osgifx.console.smartgraph.graph.Edge,
...
public class SmartGraphEdgeCurve<E, V> extends CubicCurve implements SmartGraphEdgeBase<E, V> {
    ...
    private void update() {
        if (inbound == outbound) {
            ...
            final double midpointX1 = outbound.getCenterX() - inbound.getRadius() * 5;
            final double midpointY1 = outbound.getCenterY() - inbound.getRadius() * 2;

            final double midpointX2 = outbound.getCenterX() + inbound.getRadius() * 5;
            final double midpointY2 = outbound.getCenterY() - inbound.getRadius() * 2;
            ...
            final double midpointX = (outbound.getCenterX() + inbound.getCenterX()) / 2;
            final double midpointY = (outbound.getCenterY() + inbound.getCenterY()) / 2;

            Point2D midpoint = new Point2D(midpointX, midpointY);

            final Point2D startpoint = new Point2D(inbound.getCenterX(), inbound.getCenterY());
            final Point2D endpoint   = new Point2D(outbound.getCenterX(), outbound.getCenterY());
