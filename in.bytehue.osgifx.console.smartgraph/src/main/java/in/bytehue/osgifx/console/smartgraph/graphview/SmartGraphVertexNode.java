...
public class SmartGraphVertexNode<T> extends Circle implements SmartGraphVertex<T>, SmartLabelledNode {
     ...
     */
    public SmartGraphVertexNode(final Vertex<T> v, final double x, final double y, final double radius, final boolean allowMove) {
        super(x, y, radius);
    ...
    public void moveFromForces() {
        ...
        final double height = getParent().getLayoutBounds().getHeight();
        final double width  = getParent().getLayoutBounds().getWidth();
    ...
    private void enableDrag() {
        ...
        setOnMouseDragged((final MouseEvent mouseEvent) -> {
            if (mouseEvent.isPrimaryButtonDown()) {
                final double newX = mouseEvent.getX() + dragDelta.x;
                final double x    = boundCenterCoordinate(newX, 0, getParent().getLayoutBounds().getWidth());
                setCenterX(x);

                final double newY = mouseEvent.getY() + dragDelta.y;
                final double y    = boundCenterCoordinate(newY, 0, getParent().getLayoutBounds().getHeight());
                setCenterY(y);
                mouseEvent.consume();
            }
