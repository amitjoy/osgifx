...
import static com.osgifx.console.agent.rpc.lz4.Lz4Constants.SIZE_OF_INT;
...
public final class Lz4RawDecompressor {
    ...
    public static int decompress(final Object inputBase,
                                 final long inputAddress,
                                 final long inputLimit,
                                 final Object outputBase,
                                 final long outputAddress,
                                 final long outputLimit) {
        final long fastOutputLimit = outputLimit - SIZE_OF_LONG; // maximum offset in output buffer to which it's safe
                                                                 // to write long-at-a-time
        ...
        if (outputAddress == outputLimit) {
            if (inputLimit - inputAddress == 1 && UnsafeMemory.getByte(inputBase, inputAddress) == 0) {
                return 0;
            }
            return -1;
        }
        ...
        while (input < inputLimit) {
            final int token = UnsafeMemory.getByte(inputBase, input++) & 0xFF;
            ...
            if (literalLength == 0xF) {
                ...
                int value;
                do {
                    value          = UnsafeMemory.getByte(inputBase, input++) & 0xFF;
                    literalLength += value;
            ...
            if (literalOutputLimit > (fastOutputLimit - MIN_MATCH)
                    || literalEnd > inputLimit - (OFFSET_SIZE + TOKEN_SIZE + LAST_LITERAL_SIZE)) {
                ...
                // slow, precise copy
                UnsafeMemory.copyMemory(inputBase, input, outputBase, output, literalLength);
            ...
            int offset = UnsafeMemory.getShort(inputBase, input) & 0xFFFF;
            input += SIZE_OF_SHORT;
            ...
            if (offset < SIZE_OF_LONG) {
                ...
                UnsafeMemory.putByte(outputBase, output, UnsafeMemory.getByte(outputBase, matchAddress));
                UnsafeMemory.putByte(outputBase, output + 1, UnsafeMemory.getByte(outputBase, matchAddress + 1));
                UnsafeMemory.putByte(outputBase, output + 2, UnsafeMemory.getByte(outputBase, matchAddress + 2));
                UnsafeMemory.putByte(outputBase, output + 3, UnsafeMemory.getByte(outputBase, matchAddress + 3));
                output       += SIZE_OF_INT;
                matchAddress += increment32;

                UnsafeMemory.putInt(outputBase, output, UnsafeMemory.getInt(outputBase, matchAddress));
                output       += SIZE_OF_INT;
                matchAddress -= decrement64;
            } else {
                UnsafeMemory.putLong(outputBase, output, UnsafeMemory.getLong(outputBase, matchAddress));
