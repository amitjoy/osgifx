...
import static com.osgifx.console.agent.rpc.lz4.Lz4Constants.LAST_LITERAL_SIZE;
import static com.osgifx.console.agent.rpc.lz4.Lz4Constants.MIN_MATCH;
import static com.osgifx.console.agent.rpc.lz4.Lz4Constants.SIZE_OF_LONG;
import static com.osgifx.console.agent.rpc.lz4.Lz4Constants.SIZE_OF_SHORT;
...
public final class Lz4RawCompressor {
    ...
    public static int compress(final Object inputBase,
        ...
        table[hash(UnsafeMemory.getLong(inputBase, input), mask)] = (int) (input - inputAddress);

        input++;
        int nextHash = hash(UnsafeMemory.getLong(inputBase, input), mask);
        ...
        do {
            ...
            do {
                ...
                if (nextInputIndex > matchFindLimit) {
                    return (int) (emitLastLiteral(outputBase, output, inputBase, anchor, inputLimit - anchor)
                            - outputAddress);
                }

                // get position on hash
                matchIndex = inputAddress + table[hash];
                nextHash   = hash(UnsafeMemory.getLong(inputBase, nextInputIndex), mask);

                // put position on hash
                table[hash] = (int) (input - inputAddress);
            } while (UnsafeMemory.getInt(inputBase, matchIndex) != UnsafeMemory.getInt(inputBase, input)
                    || matchIndex + MAX_DISTANCE < input);

            // catch up
            while ((input > anchor) && (matchIndex > inputAddress) && (UnsafeMemory.getByte(inputBase,
                    input - 1) == UnsafeMemory.getByte(inputBase, matchIndex - 1))) {
                --input;
                --matchIndex;
            }
            ...
            while (true) {
                ...
                table[hash(UnsafeMemory.getLong(inputBase, position), mask)] = (int) (position - inputAddress);

                // Test next position
                int hash = hash(UnsafeMemory.getLong(inputBase, input), mask);
                matchIndex  = inputAddress + table[hash];
                table[hash] = (int) (input - inputAddress);

                if (matchIndex + MAX_DISTANCE < input
                        || UnsafeMemory.getInt(inputBase, matchIndex) != UnsafeMemory.getInt(inputBase, input)) {
                    input++;
                    nextHash = hash(UnsafeMemory.getLong(inputBase, input), mask);
                    break;
                }

                // go for another match
                tokenAddress = output++;
                UnsafeMemory.putByte(outputBase, tokenAddress, (byte) 0);
            }
        } while (!done);
    ...
    private static long emitMatch(Object outputBase, long output, long tokenAddress, short offset, long matchLength) {
        // write offset
        UnsafeMemory.putShort(outputBase, output, offset);
        output += SIZE_OF_SHORT;

        // write match length
        if (matchLength >= ML_MASK) {
            UnsafeMemory.putByte(outputBase, tokenAddress,
                    (byte) (UnsafeMemory.getByte(outputBase, tokenAddress) | ML_MASK));
            long remaining = matchLength - ML_MASK;
            while (remaining >= 510) {
                UnsafeMemory.putShort(outputBase, output, (short) 0xFFFF);
                output    += SIZE_OF_SHORT;
                remaining -= 510;
            }
            if (remaining >= 255) {
                UnsafeMemory.putByte(outputBase, output++, (byte) 255);
                remaining -= 255;
            }
            UnsafeMemory.putByte(outputBase, output++, (byte) remaining);
        } else {
            UnsafeMemory.putByte(outputBase, tokenAddress,
                    (byte) (UnsafeMemory.getByte(outputBase, tokenAddress) | matchLength));
        }
    ...
    static int count(Object inputBase, final long inputAddress, final long inputLimit, final long matchAddress) {
        ...
        while (count < remaining && UnsafeMemory.getByte(inputBase, match) == UnsafeMemory.getByte(inputBase, input)) {
            count++;
            match++;
            input++;
        }
    ...
    private static long emitLastLiteral(final Object outputBase,
                                        final long outputAddress,
                                        final Object inputBase,
                                        final long inputAddress,
                                        final long length) {
        long output = encodeRunLength(outputBase, outputAddress, length);
        UnsafeMemory.copyMemory(inputBase, inputAddress, outputBase, output, length);
    ...
    private static long encodeRunLength(final Object base, long output, final long length) {
        if (length >= RUN_MASK) {
            UnsafeMemory.putByte(base, output++, (byte) (RUN_MASK << ML_BITS));

            long remaining = length - RUN_MASK;
            while (remaining >= 255) {
                UnsafeMemory.putByte(base, output++, (byte) 255);
                remaining -= 255;
            }
            UnsafeMemory.putByte(base, output++, (byte) remaining);
        } else {
            UnsafeMemory.putByte(base, output++, (byte) (length << ML_BITS));
        }
