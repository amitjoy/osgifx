...
public final class SnapshotOptionsDialogController {
    ...
    public void initData(final SnapshotOptionsDialog.SnapshotStatistics stats) {
        ...
        estimatedSizeLabel.setText("~" + formatSize(stats.estimatedUncompressedSize) + " uncompressed");
        ...
        if (stats.estimatedUncompressedSize > 100_000_000L) {
            warningLabel.setText("⚠️ This snapshot is large. Please choose an appropriate option:");
    ...
    private void createTransportLabel(final SnapshotOptionsDialog.SnapshotStatistics stats) {
    ...
    private VBox createHandlerOption(final LargePayloadHandler handler) {
        ...
        radio.setUserData(SnapshotOptionsDialog.SnapshotOption.USE_HANDLER);
    ...
    private VBox createRpcOption(final SnapshotOptionsDialog.SnapshotStatistics stats) {
        final var box = new VBox(5);

        final var radio = new RadioButton("Use RPC Transport");
        radio.setToggleGroup(toggleGroup);
        radio.setUserData(SnapshotOptionsDialog.SnapshotOption.USE_RPC);
        radio.setStyle("-fx-font-weight: bold;");
        ...
        final var recommendLabel = new Label("Recommended for snapshots < 100 MB");
    ...
    private VBox createLocalOption() {
        ...
        radio.setToggleGroup(toggleGroup);
        radio.setUserData(SnapshotOptionsDialog.SnapshotOption.STORE_LOCALLY);
        radio.setStyle("-fx-font-weight: bold;");
