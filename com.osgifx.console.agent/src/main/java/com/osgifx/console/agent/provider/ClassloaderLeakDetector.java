...
/**
 * A support for tracing classloader leaks which occur due to improper cleanup in bundles.
 *
 * <p>
 * This registers a Felix Configuration Printer which dumps out a list of suspected
 * classloaders which are not getting garbage collected. Apart from that, consumer can
 * also use {@link #reportWith(Consumer)} to report the suspected classloaders when
 * required.
 *
 * <p>
 * To detect if a classloader leak exists, the {@link #start(BundleContext)} needs to be executed
 * once and to shutdown the detection functionality, the {@link #stop()} needs to be executed.
 *
 * <p>
 * <b>IMPLEMENTATION NOTES</b>
 *
 * <p>
 * This class relies on {@link System#identityHashCode(Object)} to create a unique id for every
 * classloader instances. {@link System#identityHashCode(Object)} does not guarantee
 * that it will not generate the same number for different objects, but in practice the chance
 * of collision is rare.
 *
 * <p>
 * {@link ClassloaderLeakDetector} uses {@link PhantomReference}s to detect leaks. {@link PhantomReference}s
 * can be used to be notified when some object got out of scope to do some cleanup of resources. Remember that
 * the {@link Object#finalize()} method is not guaranteed to be called at the end of live of an object, so if
 * you need to close files or free resources, you can rely on {@link PhantomReference}s. Since these references
 * don't have a link to the actual object, a typical pattern is to derive your own Reference type from Phantom
 * and adding some info useful for the final free, for example filename. Using {@link PhantomReference}s is
 * better than overriding {@link #finalize()} and works also in those cases where {@link #finalize()} is not
 * overridable. Since in {@link SoftReference}s and {@link WeakReference}, the referent will be nullified before
 * they are enqueued in the {@link ReferenceQueue}, the garbage collector will finalize these referent at some
 * future time. On the contrary, for {@link PhantomReference}s garbage collector will finish the finalization of
 * referent and then enqueue the reference, but it will not nullify the referent.
 */
public final class ClassloaderLeakDetector implements Runnable {
    ...
    Set<XBundleDTO> getSuspiciousBundles() {
        ...
        // filter out bundles that are ACTIVE and have only one classloader created for them
