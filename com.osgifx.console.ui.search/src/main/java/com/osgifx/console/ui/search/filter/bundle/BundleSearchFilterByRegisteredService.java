...
@Component
public final class BundleSearchFilterByRegisteredService implements SearchFilter {
    ...
    public Predicate<XBundleDTO> predicate(final String input, final SearchOperation searchOperation) throws Exception {
        return switch (searchOperation) {
            case EQUALS_TO -> bundle -> {
                ...
                return registeredServices.stream().anyMatch(s -> s.objectClass.equals(input.trim()));
            };
