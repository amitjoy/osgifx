...
@Component
public final class BundleSearchFilterByWiredBundleAsProvider implements SearchFilter {
    ...
    public Predicate<XBundleDTO> predicate(final String input, final SearchOperation searchOperation) throws Exception {
        return switch (searchOperation) {
            case EQUALS_TO -> bundle -> {
                final var wiredBundlesAsProvider = bundle.wiredBundlesAsProvider;
                return wiredBundlesAsProvider.stream().anyMatch(p -> p.symbolicName.equals(input.trim()));
            };
    ...
    public String toString() {
        return "Wired Bundle as Provider";
    }
