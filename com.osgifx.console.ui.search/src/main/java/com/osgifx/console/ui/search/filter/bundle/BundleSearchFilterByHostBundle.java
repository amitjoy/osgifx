...
@Component
public final class BundleSearchFilterByHostBundle implements SearchFilter {
    ...
    public Predicate<XBundleDTO> predicate(final String input, final SearchOperation searchOperation) throws Exception {
        return switch (searchOperation) {
            case EQUALS_TO -> bundle -> bundle.hostBundles.stream()
                    .anyMatch(h -> h.symbolicName.equalsIgnoreCase(input.trim()));
            case CONTAINS -> bundle -> bundle.hostBundles.stream().anyMatch(h -> h.symbolicName.contains(input.trim()));
